...
#if NET40
            this.Page.RegisterAsyncTask(new PageAsyncTask(this.BeginLoadAsync, (ar) => { }, (ar) => { }, null));
#else
            this.Page.RegisterAsyncTask(new PageAsyncTask(async ct => await this.BeginLoadAsync(ct)));
#endif
        }

        private IAsyncResult BeginLoadAsync(object sender, EventArgs e, AsyncCallback cb, object state)
        {
            return TaskUtils.AsApm(this.BeginLoadAsync(CancellationToken.None), cb, state);
        }

        private async Task BeginLoadAsync(CancellationToken cancellationToken)
        {
