...
namespace DotNetOpenAuth.OpenId.RelyingParty
{
    ...
    public class OpenIdButton : OpenIdRelyingPartyControlBase
    {
        ...
        protected override void RaisePostBackEvent(string eventArgument)
        {
            ...
            {
#if NET40
                this.Page.RegisterAsyncTask(new PageAsyncTask(this.BeginPostBackAsync, (ar) => { }, (ar) => { }, null));
#else
                this.Page.RegisterAsyncTask(new PageAsyncTask(async ct => await this.BeginPostBackAsync(ct)));
#endif
            }
        }

        private IAsyncResult BeginPostBackAsync(object sender, EventArgs e, AsyncCallback callback, object data)
        {
            return TaskUtils.AsApm(this.BeginPostBackAsync(CancellationToken.None), callback, data);
        }

        private async Task BeginPostBackAsync(CancellationToken ct)
        {
            try
            {
